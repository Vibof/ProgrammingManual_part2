...
В одних языках программирования, таких как С, Python и многих других, индексация элементв массивов начинается с 0 (0-based). В других языках, ориентированных на математические вычисления, таких как Fortran, Matlab/Octave, R, Julia, индексация массивов начинается с 1 (1-based array).

Однако, не смотря на то, что в Julia встоенные массивы индексируются с 1, в этом языке имеется возможность определять границы диапазона индексов произвольным образом, допускается использовать даже отрицательные значения. Такие массивы представляют собой некоторую "обёртку" обычных массивов, и создаются с помощью специального конструктора `OffsetArray`, определенного в пакете `OffsetArrays.jl` (который требует предварительной установки).
...
т.е. по разным изменрениям массива пределы индексирования могут различными.

Для того, чтобы можно было программировать обобщённо, не базируясь на тот или иной способ индексирования массивов, в языке Julia предусмотрены следующие функции и зарезервированные слова:

- `eachindex`
- `enumerate`
- `first`, `firstindex`
- `last`, `lastindex`
- `begin`, `end` - в контексте индесного выражения эти слова заменяют собой значение первого и последнего индекса, соответственно.
При этом
...
Сортировка Шелла состоит в следующем. Сначала, с использованием алгоритма сотировки вставками (без бинарного поиска), упорядочиваются все элементы массива находящиеся через `n ÷ 2`  позиций (`n` - длина массива), затем - все элементы, стоящие через `n ÷ 4`, и т.д., и, наконец, все элементы стоящие через 1 позицию. В результате массив оказывается отсортированным.
...
Таким образом в классическом алгоритме Шелла используется последовательность промежутков, равных $n\div 2^i$, где $i = 1,2,...,k$ ($k$ - наибольшее натуральное число, такое что $2^k \le n$). В этом случае вычислительная сложность алгоритма оценивается как $O(n^2)$.

Однако известны другие способы выбора величин этих промежутков, позволяющие существенно понизить оценку вычислительной сложности этого алгоритма сортировки, см., например, [здесь](https://ru.wikipedia.org/wiki/Сортировка_Шелла).
