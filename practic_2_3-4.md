...
Однако, не смотря на то, что в Julia встоенные массивы индексируются с 1, в этом языке имеется работать с массивами, котрые индексированы каким угодно образом, не обязательно даже с 0 или 1, но возможна даже индексация и с какого-либо отрицательного значения. Такие массивы представляют собой специальную "обёртку" обычных массивов и создаются с помощью специального конструктора `OffsetArray`, определенного в отдельном пакете `OffsetArrays.jl` (который требует предварительной установки).

Например, если имеется стандартный одномерный массив `A` некоторого существующего типа, тогда можно создать новую **ссылку** (типа `OffsetArray`) на этот же самый массив, но с другим диапазоном значений индексов, например, с диапазоном индексов `0:length(A)-1`:
...
Тут границы диапазона индексов могли бы быть и любыми другими, лишь бы число индексов из диапазона совпадало с длиной массива `A`.
