...
г) с использованием массиванием массива длины `n/2` - это возможно с учетом свойства симметрии биномиальных коэффициентов (в этом случае должна возвращаться только первая половина набора всех биномиальных коэффициентов заданного порядка).
...
1. Для вычисления базисных функций Бернштейна вывести рекуррентную формулу, основанную на рекуррентном соотношении для биномиальных коэффициентов:
$$
C_n^k=C_{n-1}^{k-1}+C_{n-1}^{k}
$$
(в результате должен получиться фактически алгоритм де Кастельжо, описанный, например, [здесь](https://ru.wikipedia.org/wiki/Алгоритм_де_Кастельжо)). Однако здесь можно было бы и непосредственно воспользоваться результатом задачи 4, но это было бы чуть менее эффективно.

2. Вместо того, чтобы передавать через соответствующий параметр функцию `f` в нашу программу (функцию), можно было бы передавать в нее сразу последовальность соответствующих коэффициентов (при базисных функциях Бенштейна, см. определение многочлена Бернштейна). В этом случае следовало бы допустить, чтобы эта последовательность коэффициентов могла бы задаваться или соответствующим вектором, или кортежем, или генератором. В этом случае тип этого аргумента в заголовке нашей функции лучше всего вообще не аннотировать. Тогда, если сначала определить функцию с таким аргументом, то на её основе можно буквально в одну строчку будет определить и требуемую функцию высшего порядка, и, кроме того, в таком виде она могла бы быть непосредственно использована, при необходимости, для построения так называемых кривых Безье (см. ссылку выше).

3. Для тестирования полученного алгоритма убедиться в справедливости сформулированноой выше теоремы путём построения соответствующих графиков (речь идет просто о графической иллюстрации утверждения теоремы). Т.е. требуется выбрать какую-либо непрерывную на отрезке [0;1] функцию, и построить её график вместе с семейством графиков, апроксимирующих её многочленов Бернштейна порядков `1,2,3,...N` (`N` - выбрать таким, чтобы добиться требуемой наглядности).