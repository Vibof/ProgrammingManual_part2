...
При этом удобно будет сразу считать, что передаваемая в нашу программу (через соответствующий параметр) функция `f` есть функция целочисленного аргумента `k` (см. соответствующую формулу) - в дальнейшем это позволило бы, например, использовать эту функцию непосредственно для построения графиков так называемых кривых Безье (см. ссылку ниже).
